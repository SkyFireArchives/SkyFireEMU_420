...
// The below block of codes have been commented out. The reason is that they are
// actually never used AND cause errors the way they are. Clearly someone had
// something on their mind but left things half completed for unix systems for
// a long time. Some lines of code are also commented out from
// void CliRunnable::run() using block comments due to similar reasons.
/*
...
#if PLATFORM != PLATFORM_WINDOWS
        free(command_str); // We are responsible for freeing memory allocated by readline.
#endif
    }
}